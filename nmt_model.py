...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # check the size of the resiulting tensor 
        # print(X.size())
        packed_embed = nn.utils.rnn.pack_padded_sequence(input=X, 
                                                         lengths=source_lengths, 
                                                         batch_first=False)
        #print(packed_embed.data.size())
        enc_hiddens, (last_hidden, last_cell) = self.encoder(packed_embed)
        #print(last_hidden.size())
        #print(last_cell.size())
        enc_hiddens_pad = nn.utils.rnn.pad_packed_sequence(enc_hiddens)
        
        # Takes out the result from the package object created before. 
        enc_hiddens = enc_hiddens_pad[0]
        #print('enc_hiddens size:', enc_hiddens.size())
        enc_hiddens = torch.transpose(enc_hiddens, 0, 1)
        #print('enc_hiddens size:', enc_hiddens.size())

        # The last 1 corresponds to the dimension where the concatenation is being done.
        ...
        #print('last_cell size:', last_hidden.size())
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        alpha_t_t = torch.unsqueeze(alpha_t,dim=1)
        #print('alpha_t_t', alpha_t_t.size())
        a_t = torch.bmm(alpha_t_t, enc_hiddens)
